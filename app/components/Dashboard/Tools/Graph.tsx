...
const Graph: FC<GraphProps> = ({ graphId }) => {
  const { data: graphData } = useGetGraphDataQuery(graphId);
  // Define months
  const months = [
    "Jan",
    "Feb",
  ...
  // Map backend data to graph format
  const data = months.map((month, index) => {
    const item =
      graphData?.recent_exchange_rates[
        index % graphData?.recent_exchange_rates.length
      ]; // Use modulo to loop through backend data
    return {
